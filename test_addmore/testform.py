...
class Ui_add_Data(object):
        ...
        if selected_rows:
            for selected_row in sorted(selected_rows, reverse=True):
                ...
                try:
                    ...
                    # Xoá toàn bộ thư mục từ Firebase Storage
                    prefix = f"{student_id}/"
    ...
    def run_EncodeGenerator(self):
        ...
        encode_generator_path = os.path.join(current_directory, "test_encode.py")
        print(encode_generator_path)
    ...
    def add_data_to_firebase(self):
        ...
        file_paths, _ = file_dialog.getOpenFileNames(None, "Chọn các file ảnh", "","Image files (*.png;*.jpg;*.jpeg;*.gif)")
        ...
        try:
            ...
            for i, file_path in enumerate(file_paths, 1):
                image_name = f"{student_id}_{i}.png"
                with open(file_path, "rb") as file:
                    ...
                    resized_image_path = os.path.join(tempfile.gettempdir(), f"{student_id}_{i}.png")
                    img.save(resized_image_path, "PNG")

                    # Upload the resized image to Firebase Storage
                    blob = bucket.blob(f"{student_id}/{image_name}")
                    blob.upload_from_filename(resized_image_path, content_type="image/png")

            print("Data and images added successfully!")
